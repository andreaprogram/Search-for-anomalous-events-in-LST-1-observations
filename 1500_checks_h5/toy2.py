    ...
    for p in range(y.shape[1]):
        ...
        try:
            ...
            if len(model.params) < 2:
                raise ValueError("Robust fit returned only one parameter")
...
def f1(subruns, event_rate, event_rate_sigma, sigma_cutoff):
    ...
    if not event_rate_robfit["valid"][0]:
        ...
        return {
            ...
            "good_subruns": event_rate_robfit["good_points"][0][0],
            "good_event_rate": event_rate_robfit["good_points"][0][1]
        }
...
def f2(subruns, cdf, sigma_cutoff):
    ...
    if not D_robust_fit["valid"][0]:
        ...
        return {
            ...
            "intercept": D_robust_fit["intercept"][0],
            "u_intercept": D_robust_fit["u_intercept"][0],
            "good_subruns": D_robust_fit["good_points"][0][0],
            "good_D": D_robust_fit["good_points"][0][1]
        }
...
def main():
    ...
    for file in file_list:
        ...
        try:
            ...
            errored_runs.append((file, str(e)))
            continue

        run_str = re.search(r"Run(\d+)", a.filename).group(1)  # saves correctly the run id's starting on 0, int() takes the numerical value
...
# FILTERS TO DETECT ANOMALIES-----------------------------------------------------------------------------------------------------------------------------------
            ...
            dictionary.update({
                ...
                "good_x_1" : result_1["good_subruns"],
                "good_y_1" : result_1["good_event_rate"]
            })
            ...
            dictionary.update({
                ...
                "good_x_2": result_2["good_subruns"],
                "good_y_2": result_2["good_D"],
    ...
    # CREATION OF AN h5 FILE TO STORE THE RELATED DATA FOR THE INTERESTING SUBRUNS----------------------------------------------------------------------------------
            ...
            class FitData(tables.IsDescription):
                valid = tables.BoolCol()
                slope = tables.Float64Col()
                u_slope = tables.Float64Col()
                intercept = tables.Float64Col()
                u_intercept = tables.Float64Col()  

            class GoodPoints(tables.IsDescription):
                x = tables.Int32Col()
                y = tables.Float64Col()
            ...
            for f in filters:
                ...
                # FIT TABLE----------------------------
                t_fit = checks_h5.create_table("/perfilter", "filter{f}_fit".format(f=f), FitData, "Fit Filter {f}".format(f=f))
                row_fit = t_fit.row
                
                
                row_fit["valid"] = results[f]["valid"]
                row_fit["slope"] = dictionary[f"slope_{f}"]
                row_fit["u_slope"] = dictionary[f"u_slope_{f}"]
                row_fit["intercept"] = dictionary[f"intercept_{f}"]
                row_fit["u_intercept"] = dictionary[f"u_intercept_{f}"]
                row_fit.append()

                # GOOD POINTS TABLE--------------------
                t_points = checks_h5.create_table("/perfilter", f"filter{f}_points", GoodPoints, f"Selected points for Filter {f}")
                row_points = t_points.row  # Corregido: creamos row_points
            
                if f in ["1", "2"]:
                    good_x = dictionary.get(f"good_x_{f}")
                    good_y = dictionary.get(f"good_y_{f}")
            
                    if good_x is None or good_y is None or len(good_x) == 0:
                        row_points["x"] = -1
                        row_points["y"] = np.nan
                        row_points.append()
                    else:
                        for x, y in zip(good_x, good_y):
                            row_points["x"] = x
                            row_points["y"] = y
                            row_points.append()
                else:
            ...
            # any unexpected failure for this run (missing table, malformed
            # column, etc.) gets logged here instead of crashing the whole batch
            errored_runs.append((run_str, str(e)))
            
        finally:
# always close whatever got opened for this run, even on failure
            a.close()
            if checks_h5 is not None:
                checks_h5.close()
