# THE AIM OF THIS CODE IS TO APPLY FILTER 3 TO POTENTIALLY INTERESTING SUBRUNS / THE ONES WITH A HIGHER NUMBER OF WHAT WE'LL CALL 'ANOMALOUS PIXELS' I.E. THE ONES WITH FLUCTUATIONS OF THE RATE OF COG
# UNCOMPATIBLE WITH THE TENDENCY OF THIS QUANTITY ALONG THE WHOLE RUN. 
...
# CREATION OF AN h5 FILE TO STORE THE RELATED DATA FOR THE SUBRUNS WITH POTENTIAL STRAIGHT LINES----------------------------------------------------------------------------------
        ...
        # - CHI2
        # - p-value
        # - z-score of the selected pixels (?)

        spath_h5.create_group("/", "filter3", "Straight-path candidates")

        class StraightPath(tables.IsDescription):
        ...
        row = t.row
        for c, chi2_data, p, cen, vec in zip(
                dictionary["checks_3"],
                dictionary["chis2"],
                dictionary["p_values"],
                dictionary["centroids"],
                dictionary["direction_vectors"]):
        
            row["check_index"] = c
            row["chi2"] = chi2_data
            row["p_value"] = p
            row["centroid"] = cen
            row["direction_vector"] = vec
            row.append()
