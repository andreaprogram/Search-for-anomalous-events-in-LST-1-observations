...
# ROBUST FIT FUNCTION - PCA (Principal Component Analisis)
def robust_fit_pca(x, y, z_thresh=3.5):
    pts = np.column_stack((x, y))
    fit = np.zeros_like(pts)
    ...
    c = pca.mean_ #centroid
    v = pca.components_[0] # direction vector

    # Orthogonal distance from each point to the direction vector
    proj = np.dot(pts - c, v)
    proj_pts = c + np.outer(proj, v)
    residual = np.linalg.norm(pts - proj_pts, axis=1)
    ...
    if good.sum() >= 0.6 * len(x):
        ...
        good_points.append(None)

        fit[:] = np.nan
        centroid.append(np.array([np.nan, np.nan]))
        direction.append(np.array([np.nan, np.nan]))
